...
void populateScene(Scene& scene)
{
    ...
    const auto random_color = [&]() {
        return 0.5f * (1 + dis(gen));
    };

    for (int i = -11; i < 11; ++i) {
        for (int j = -11; j < 11; ++j) {
            Vec3f center {i + 0.9f * dis(gen), 0.2f + 0.1f * dis(gen), j + 0.9f * dis(gen)};
...
try {
    ...
    Camera camera{Vec3f{3, 4, 1}, Vec3f{2, 0, 0}, Vec3f{0, 1, 0},
